...
public:
    MoveFlying() : plansys2::ActionExecutorClient("move_flying", 7000ms) { //modify here
        progress_ = 0.0;
    }
...
private:
    void do_work() {
        ...
        std::cout << "MoveFlying executed ... [" << std::min(100.0, progress_ * 100.0) << "%]  " << std::flush; //modify here
    }
    float progress_;
};
