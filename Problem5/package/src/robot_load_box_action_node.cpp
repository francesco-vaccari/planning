...
public:
    RobotLoadBox() : plansys2::ActionExecutorClient("robot_load_box", 200ms) { //modify here
        progress_ = 0.0;
    }
...
private:
    void do_work() {
        ...
        std::cout << "RobotLoadBox executed ... [" << std::min(100.0, progress_ * 100.0) << "%]  " << std::flush; //modify here
    }
    float progress_;
};
